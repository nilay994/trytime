...
## Floods local network with synthetic NTP packets, potentially disrupting
## clocks of computers instructed to fetch time from network 
    ...
    def getCommandlineOpts(self,argv):
        ...
        for opt, arg in opts:
            if opt in ("-h", "--help"):
                ...
                print(datetime(1994, 9, 16, 5, 40))
                dt = datetime(1994, 9, 16, 5, 40)
                self.offset = (datetime(1900, 1, 1))
            else:
                self.usage()
                sys.exit(1)
